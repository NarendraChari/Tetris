...
export default function Tetris() {
  ...
  }, [moveDown, gameOver, dropTime, gameStarted])
  ...
  const getColorStyle = (color) => {
    ...
    return color ? colorMap[color] || "#d1d5db" : "#f3f4f6"
  }
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100">
            ...
            row.map((_, x) => (
              <AnimatePresence key={`${y}-${x}`}>
                <motion.div
                  ...
                  style={{
                    border: "1px solid #e5e7eb",
                    backgroundColor: getColorStyle(renderCell(x, y)),
                  }}
                  className="w-5 h-5"
                />
              </AnimatePresence>
      ...
      <div className="flex gap-4 mt-4">
        ...
        {(gameStarted || gameOver) && <Button onClick={resetGame}>{gameOver ? "Play Again" : "Reset Game"}</Button>}
